...
type ClassificationViewRouteProp = RouteProp<RootStackParamList, 'ClassificationView'>;

type Props = { route: ClassificationViewRouteProp };

const ClassificationView: React.FC<Props> = ({ route }) => {
  const { phaseId, tournamentId } = route.params;
  const {
    classificationRows,
    sortedStats,
    loading,
  ...
  } = useClassificationViewModel(phaseId, tournamentId);
  ...
  const renderTabBar = () => (
    <View style={styles.tabBar}>
      {['Classificação', 'Partidas'].map((tab) => {
    ...
    if (!classificationRows || classificationRows.length === 0) {
      return (
        <View style={styles.emptyState}>
          <Text style={styles.emptyText}>Sem dados de classificação disponíveis.</Text>
        </View>
      );
    }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
      {renderTabBar()}
      {activeTab === 'Classificação' ? renderClassification() : renderPartidas()}
    </View>
  );
};
